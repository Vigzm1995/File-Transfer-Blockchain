...
function get_my_addr(){
    ...
    if (no_displayed_error_result($getaddresses, multichain('getaddresses', true))) {
        ...
        if (no_displayed_error_result($listpermissions,
            multichain('listpermissions', 'send', implode(',', array_keys($addressmine)))
        )){
           foreach ($listpermissions as $listpermission){
               array_push($my_addr,$listpermission['address']);
           }
        }else{
        ...
        if(count($my_addr)!=0)
            ...
            $ak = array_keys($addressmine);
            array_push($ak,"No permissions");
        }
            return $ak;
    }
